...
import io.rocketpartners.rql.sql.SqlReplacer;
...
public class TestSqlRql extends TestCase
{
   ...
   public void test1() throws Exception
   {
      ...
      tests.add(new RqlTest("w(city,andl)", //
                            "select * from table1", //
                            null, //
                            "select * from table1 WHERE `city` LIKE ?", //
                            "city", "'%andl%'"));
      ...
      tests.add(new RqlTest("sum(if(eq(`col1`,'value'),1,'something blah'))", //
                            ...
                            "SELECT *, SUM(IF(`col1` = ?, 1, ?)) from table1", //
      ...
      tests.add(new RqlTest("sum(if(eq(\"col1\",if(ne(`col2`,'val1'),`col3`,'val2')),1,'something blah'))", //
                            ...
                            "SELECT *, SUM(IF(`col1` = IF( NOT `col2` <=> ?, `col3`, ?), 1, ?)) from table1", //
      ...
      tests.add(new RqlTest("like(name,asdf*)", //
                            "SELECT * FROM Person", //
                            "SELECT * FROM Person WHERE `name` LIKE 'asdf%'", //
